...
export function FAQ() {
  ...
  return (
    <section className="py-24 bg-luxe-blush">
      <div className="max-w-3xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-serif text-luxe-dark mb-4">Common Inquiries</h2>
          <p className="text-luxe-dark/70">Everything you need to know before your Bougie experience.</p>
        </div>

        <div className="space-y-4">
          {faqs.map((faq, i) => (
            <div key={i} className="border-b border-luxe-dark/10 last:border-0 pb-4">
              <button 
                onClick={() => setOpenIndex(openIndex === i ? null : i)}
                className="w-full flex justify-between items-center py-4 text-left group"
              >
                <span className="text-lg font-medium text-luxe-dark transition-colors">
                  {faq.question}
                </span>
                {openIndex === i ? <Minus className="w-5 h-5 text-luxe-plum" /> : <Plus className="w-5 h-5 text-luxe-dark/30" />}
              </button>
              {openIndex === i && (
                <div className="pb-4 animate-in fade-in slide-in-from-top-2 duration-300">
                  <p className="text-luxe-dark/70 leading-relaxed">{faq.answer}</p>
                </div>
              )}
            </div>
          ))}
