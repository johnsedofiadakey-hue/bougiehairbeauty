...
export function Testimonials() {
  ...
  return (
    <section ref={revealRef} className="py-24 bg-luxe-dark/5 reveal">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <div className="flex justify-center gap-1 mb-4">
             {[...Array(5)].map((_, i) => <Star key={i} className="w-5 h-5 fill-luxe-plum text-luxe-plum" />)}
          </div>
          <h2 className="text-4xl md:text-5xl font-serif text-luxe-dark mb-4">Client Love</h2>
          <p className="text-luxe-dark/60 max-w-xl mx-auto">Hear from those who have experienced the Bougie transformation.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map((review, i) => (
            <div
              key={i}
              className="bg-white p-10 rounded-3xl shadow-sm border border-luxe-dark/10 relative group hover:shadow-xl transition-all duration-500"
            ...
            >
              <div className="w-14 h-14 rounded-2xl glass-dark flex items-center justify-center absolute top-8 right-8 group-hover:scale-110 transition-transform">
                <Quote className="w-6 h-6 text-luxe-blush" />
              </div>
              <div className="flex gap-1 mb-6">
                {[...Array(review.rating)].map((_, j) => <Star key={j} className="w-3 h-3 fill-luxe-plum text-luxe-plum" />)}
              </div>
              <p className="text-luxe-dark/70 mb-8 italic leading-relaxed">&quot;{review.content}&quot;</p>
              <div>
                <p className="font-bold text-luxe-dark">{review.name}</p>
                <p className="text-xs text-luxe-dark/50 uppercase tracking-widest">{review.role}</p>
              </div>
            </div>
          ))}
        </div>
